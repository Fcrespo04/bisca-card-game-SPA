...
class HistoryController extends Controller
{
    ...
    public function leaderboardGlobal(Request $request)
    {
        ...
        $topMatches = User::select(
            'users.nickname',
            'users.photo_avatar_filename',
            DB::raw('count(T_M.id) as total_wins'),
            DB::raw('MAX(T_M.ended_at) as last_win_date'), // Data da última vitória
            DB::raw('T_S.total_score')
        )
            ->from('users')
            ->join('matches AS T_M', 'users.id', '=', 'T_M.winner_user_id')
            ->joinSub($scoresQuery, 'T_S', 'users.id', '=', 'T_S.id')
            ...
            ->orderBy('last_win_date', 'asc') // 👑 DESEMPATE: Quem ganhou primeiro fica em cima
            ->limit(10)
            ->get();

        // --- 3. Top 10 por Vitórias em Jogos Parciais (Games) ---
        $topGames = User::select(
            'users.nickname',
            'users.photo_avatar_filename',
            DB::raw('count(T_G.id) as total_wins'),
            DB::raw('MAX(T_G.ended_at) as last_win_date'), // Data da última vitória
            DB::raw('T_S2.total_score')
        )
            ->from('users')
            ->join('games AS T_G', 'users.id', '=', 'T_G.winner_user_id')
            ->joinSub($scoresQuery, 'T_S2', 'users.id', '=', 'T_S2.id')
            ...
            ->orderByDesc('total_wins')
            ->orderBy('last_win_date', 'asc') // 🃏 QUEM CHEGOU PRIMEIRO
            ->limit(10)
            ->get();
        ...
        $topScorers = DB::table('games')
            ->select(
                ...
                DB::raw('MAX(games.ended_at) as last_point_date') // Quando atingiu o score
            )
            ->join('users', function ($join) {
                $join->on('users.id', '=', 'games.player1_user_id')
                    ->orOn('users.id', '=', 'games.player2_user_id');
            ...
            ->orderBy('last_point_date', 'asc') // 🎯 QUEM ATINGIU PRIMEIRO
            ->limit(10)
            ->get();
    ...
    public function statisticsPlayer(Request $request, User $user)
    {
        ...
        // 💡 CORREÇÃO APLICADA: SE ESTIVER DESATIVADO, DEVOLVE 200 OK COM MENSAGEM
        if ($user->deleted_at !== null) {
            // Devolve 200 OK com um objeto de erro para que o Frontend (Vue) não dispare o catch
            // e possa exibir a mensagem na interface.
            return response()->json([
                'error' => 'Conta desativada/removida. Sem estatísticas disponíveis.',
                'matches_played' => 0,
                'matches_won' => 0,
                'games_played' => 0,
                'games_won' => 0,
        ...
        // O restante do cálculo de estatísticas SÓ corre se a conta estiver ativa
        // --- CÁLCULO DO SCORE TOTAL (Multiplayer) ---
        $totalScore = Game::where(function ($q) use ($id) {
            // ... (cálculo de score mantido)
        })
            ->where('status', 'Ended')
